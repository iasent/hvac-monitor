...
class PMS7003(object):
    ...
    def get(self):
        ...
        if not self.protocol_chk(buffer):
            raise Exception('illegal response from pms7003')

        data = self.unpack_data(buffer)

        # cf1 = 표준 대기  ** Note: CF=1 should be used in the factory environment
        # atm(atmosphere) = 주변 대기 환경

        # PM* 단위는 ug/m3

        return {
            'pm1.0': data[self.DUST_PM1_0_ATM],
            'pm2.5': data[self.DUST_PM2_5_ATM],
            'pm10.0': data[self.DUST_PM10_0_ATM],
            '0.3um_in_0.1L_of_air': data[self.DUST_AIR_0_3],
            '0.5um_in_0.1L_of_air': data[self.DUST_AIR_0_5],
            '1.0um_in_0.1L_of_air': data[self.DUST_AIR_1_0],
            '2.5um_in_0.1L_of_air': data[self.DUST_AIR_2_5],
            '5.0um_in_0.1L_of_air': data[self.DUST_AIR_5_0],
            '10.0um_in_0.1L_of_air': data[self.DUST_AIR_10_0]
        }
